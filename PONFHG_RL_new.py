...
def get_reward(state, action, partial_ordering, course_list_original_train, learner_id):
    ...
    if action in partial_ordering:
        ...
        # Get all previous courses in the ordering before the current action
        previous_courses = partial_ordering[:action_index]  # All courses prior to the current action

        # Get the list of completed courses for the current learner
        completed_courses = course_list_original_train.get(learner_id, [])

        # Count how many previous courses the user has completed
        completed_count = sum(course in completed_courses for course in previous_courses)

        # Reward based on the number of preceding courses completed
        reward += completed_count * 0.5  # Example: 0.2 reward per completed course

    # Check if the user has completed this course before (based on real data)
    if action in course_list_original_train.get(learner_id, []):
        reward += 0.8  # Reward for recommending a course the learner has completed before
    ...
    for epoch in range(epochs):
        for learner in df_train['learner_id']:
            ...
            if len(learner_courses) > 0:
                ...
                '''
                reward = get_reward(state, action, partial_ordering, course_list_original_train, learner_id)
